  ...
  return (
    <section id="faq" className="bg-brand-tint py-20 md:py-28">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20">
        <Reveal><span className="text-sm font-semibold uppercase tracking-[0.14em] text-brand">Vos questions</span><h2 className="mt-3 text-3xl font-semibold leading-tight text-ink md:text-4xl">Répondre aux vraies questions avant de commencer.</h2><p className="mt-5 text-ink-soft">Vous hésitez ? La première prise de contact sert aussi à vous orienter avec honnêteté.</p></Reveal>
        <div className="divide-y divide-brand-light border-y border-brand-light">
          {questions.map(([question, answer], i) => <Reveal key={question} delay={i * 45}><details className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-base font-semibold text-ink"><span>{question}</span><span className="text-2xl font-light text-brand transition-transform group-open:rotate-45">+</span></summary><p className="max-w-2xl pt-3 leading-relaxed text-ink-soft">{answer}</p></details></Reveal>)}
        </div>
      </div>
    </section>
  );
